...
{% macro header(item) %}
	<div class="price__header">		
		{% if item %} 
			<div class="price__title">Спасибо за&nbsp;регистрацию!</div>
		{% else %}
...
{% macro offline(wrap_class) %}
    {% if wrap_class %}<div class="payments__wrapper {{wrap_class}}">{% endif %}
    {% for item in offline_packages %}
    {{ card(item) }}
    {% endfor %}
    {% if wrap_class %}</div>{% endif %}
{% endmacro %}
